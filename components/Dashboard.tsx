...
import { buildDashboardMetrics, ClientRisk, DashboardMetrics, FeedItem, HygieneIssue, PipelineMetric, WorkloadRow } from '../services/dashboardMetricsService';
...
const RiskQueue: React.FC<{ risks: ClientRisk[]; onOpenTasks: () => void }> = ({ risks, onOpenTasks }) => (
    ...
    {risks.length === 0 ? (
      ...
      <div className="divide-y divide-white/[0.05] max-h-[520px] overflow-y-auto">
        {risks.slice(0, 12).map(risk => {
          ...
          return (
            <button key={risk.board.id} onClick={onOpenTasks} className="w-full px-5 py-3.5 hover:bg-portal-surface2 transition-colors text-left">
...
const TeamCapacity: React.FC<{ rows: WorkloadRow[] }> = ({ rows }) => {
  const maxLoad = rows.reduce((max, row) => Math.max(max, row.openCount), 1);
  return (
    <Panel title="Team Capacity" subtitle="Open, overdue and completed this week" icon={<Users className="w-4 h-4" />} className="lg:col-span-6">
      {rows.length === 0 ? (
        ...
        <div className="divide-y divide-white/[0.05] max-h-[460px] overflow-y-auto">
          {rows.slice(0, 12).map(row => {
            ...
            return (
              <div key={row.profile.id} className="px-5 py-3.5">
                <div className="flex items-center justify-between gap-3 mb-2">
                  <div className="min-w-0">
...
const DueSoonPanel: React.FC<{ metrics: DashboardMetrics; onOpenTasks: () => void }> = ({ metrics, onOpenTasks }) => (
    ...
    {metrics.dueThisWeekTasks.length === 0 ? (
      ...
      <div className="divide-y divide-white/[0.05] max-h-[360px] overflow-y-auto">
        {metrics.dueThisWeekTasks.slice(0, 12).map(item => (
          <button key={`${item.boardId}-${item.task.id}`} onClick={onOpenTasks} className="w-full px-5 py-3 flex items-center justify-between gap-4 text-left hover:bg-portal-surface2 transition-colors">
            <div className="min-w-0">
...
const Dashboard: React.FC<DashboardProps> = ({ currentUser, setCurrentView }) => {
  ...
  const openTasksView = () => setCurrentView(ToolView.TASKS);
  ...
  return (
    <div className="p-4 md:p-6 max-w-[1600px] mx-auto space-y-6">
      ...
      <div className="grid grid-cols-2 md:grid-cols-3 xl:grid-cols-6 gap-4">
        <KpiCard label="Active Clients" value={metrics.boards.length} detail="Live client boards" tone="brand" icon={<Users className="w-5 h-5" />} onClick={openTasksView} />
        <KpiCard label="Open Tasks" value={metrics.openTasks.length} detail="Excludes Done groups and done statuses" tone="blue" icon={<ListChecks className="w-5 h-5" />} onClick={openTasksView} />
        <KpiCard label="Overdue" value={metrics.overdueTasks.length} detail={`${metrics.clientRisks.filter(r => r.overdueCount > 0).length} affected clients`} tone={metricTone(metrics.overdueTasks.length, 25, 100)} icon={<Flame className="w-5 h-5" />} onClick={openTasksView} />
        <KpiCard label="Due This Week" value={metrics.dueThisWeekTasks.length} detail={`${metrics.dueTodayTasks.length} due today`} tone="brand" icon={<Clock3 className="w-5 h-5" />} onClick={openTasksView} />
        <KpiCard label="Blocked" value={metrics.blockedTasks.length} detail="Waiting, stuck or blocked language" tone={metricTone(metrics.blockedTasks.length, 10, 30)} icon={<AlertTriangle className="w-5 h-5" />} onClick={openTasksView} />
        <KpiCard label="Completed" value={metrics.completedThisWeekTasks.length} detail="Completed this week" tone="green" icon={<CheckCircle2 className="w-5 h-5" />} onClick={openTasksView} />
      </div>

      <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
        <RiskQueue risks={metrics.clientRisks.filter(r => r.overdueCount > 0 || r.level !== 'Watch').slice(0, 20)} onOpenTasks={openTasksView} />
        <PipelineHealth pipelines={metrics.pipelineMetrics} />
        <AgencyFeed items={metrics.feedItems} />
      </div>

      <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
        <TeamCapacity rows={metrics.workloadRows} />
        <AiBriefing briefing={metrics.briefing} />
        <HygienePanel issues={metrics.hygieneIssues} />
      </div>

      <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
        <DueSoonPanel metrics={metrics} onOpenTasks={openTasksView} />
