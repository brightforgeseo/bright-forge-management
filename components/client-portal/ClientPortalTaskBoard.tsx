...
const COLUMNS: { id: PartnerTaskStatus; title: string; color: string; icon: React.ElementType }[] = [
  ...
  { id: 'needs_content', title: 'Needs Content', color: 'bg-portal-accent', icon: FileText },
...
const ClientPortalTaskBoard: React.FC<ClientPortalTaskBoardProps> = ({
      ...
      <div className="p-8">
        <div className="animate-pulse space-y-8">
          ...
          <div className="flex gap-4 overflow-x-auto">
            {[1, 2, 3, 4, 5].map((i) => (
              <div key={i} className="w-72 flex-shrink-0 h-96 bg-portal-surface2 rounded-xl"></div>
            ))}
          </div>
        </div>
      </div>
  ...
  return (
    <div className="h-full flex flex-col bg-portal-dark">
      {/* Header */}
      <div className="p-6 border-b border-white/[0.07] bg-portal-surface/50 backdrop-blur-sm sticky top-0 z-10">
          ...
          <div className="flex items-center gap-3">
            ...
            <div className="relative">
              ...
              <input
                ...
                className="pl-9 pr-4 py-2 bg-portal-surface2 border border-white/[0.07] rounded-lg text-white placeholder-portal-soft focus:outline-none focus:border-portal-accent w-48 lg:w-64"
              />
              {searchQuery && (
                <button
                  onClick={() => setSearchQuery('')}
                  className="absolute right-3 top-1/2 -translate-y-1/2 text-portal-soft hover:text-white"
                >
                  <X className="w-4 h-4" />
                </button>
              )}
            ...
            <div className="relative" ref={dropdownRef}>
              <button
                ...
                className="flex items-center gap-2 px-4 py-2 bg-portal-surface2 border border-white/[0.07] rounded-lg text-white hover:bg-portal-surface2 transition-colors"
              >
              ...
              {showClientDropdown && (
                <div className="absolute right-0 mt-2 w-64 bg-portal-surface2 border border-white/[0.07] rounded-xl shadow-xl z-20 overflow-hidden">
                  <button
                    onClick={() => {
                      setSelectedClient(null);
                      setShowClientDropdown(false);
                    }}
                    className={`w-full px-4 py-3 text-left hover:bg-portal-surface2 transition-colors ${
                      !selectedClient ? 'bg-portal-surface2 text-portal-accent' : 'text-white'
                    }`}
                  >
                    All Clients
                  </button>
                  {clients.map((client) => (
                    <button
                      ...
                      className={`w-full px-4 py-3 text-left hover:bg-portal-surface2 transition-colors flex items-center gap-3 ${
                        selectedClient === client.id ? 'bg-portal-surface2 text-portal-accent' : 'text-white'
                      }`}
                    >
                      <div
        ...
        <div className="flex gap-4 min-w-max">
          {COLUMNS.map((column) => {
            ...
            return (
              <div
                key={column.id}
                className={`w-72 flex-shrink-0 bg-portal-surface rounded-xl border transition-all ${
                  isOver ? 'border-portal-accent bg-orange-950' : 'border-white/[0.07]'
                }`}
              ...
              >
                ...
                <div className="p-4 border-b border-white/[0.07] flex items-center gap-3">
                  ...
                  <div className="flex-1">
                    ...
                    <p className="text-xs text-portal-soft">{columnTasks.length} tasks</p>
                  </div>
                </div>
                  ...
                  {columnTasks.length === 0 ? (
                    ...
                    columnTasks.map((task) => (
                      <div
                        ...
                        className={`bg-portal-surface2 rounded-xl p-4 cursor-grab border border-white/[0.07] hover:border-portal-accent/50 hover:bg-portal-surface2 transition-all select-none ${
                          draggedTask?.id === task.id ? 'opacity-50 cursor-grabbing' : ''
                        }`}
                      >
                        <div className="flex items-start gap-2 mb-2">
                          <GripVertical className="w-4 h-4 text-portal-soft flex-shrink-0 mt-1 cursor-grab active:cursor-grabbing" />
                          <div className="flex-1 min-w-0">
                            <p className="font-medium text-white text-sm line-clamp-2">
                              {task.task?.title || `Task ${task.task_id.slice(0, 8)}`}
                            </p>
                            {task.client_name && (
                              <p className="text-xs text-portal-soft mt-1">{task.client_name}</p>
                            )}
                          </div>
                        </div>

                        {task.visible_notes && (
                          <p className="text-xs text-portal-soft mt-2 line-clamp-2 ml-6">
                            {task.visible_notes}
                          </p>
                        )}
                        ...
                        {task.task?.dueDate && (
                          <div className="flex items-center gap-1 mt-3 ml-6">
                            <Clock className="w-3 h-3 text-portal-soft" />
                        ...
                        {task.task?.attachments && task.task.attachments.length > 0 && (
                          <div className="flex items-center gap-1 mt-3 ml-6 text-xs text-portal-soft">
                            <Paperclip className="w-3 h-3" />
                            <span>{task.task.attachments.length} attachment{task.task.attachments.length === 1 ? '' : 's'}</span>
                          </div>
                        )}
                        ...
                        <div className="flex items-center justify-between mt-3 ml-6">
                          <div className="flex gap-2">
                            {task.task?.worksheet && (
                              <a
                                ...
                                className="flex items-center gap-1 text-xs text-portal-accent hover:text-orange-300"
                          ...
                          {task.task?.comments && task.task.comments.length > 0 && (
                            <div className="flex items-center gap-1 text-xs text-portal-soft">
