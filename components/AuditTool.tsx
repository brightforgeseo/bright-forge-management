  ...
  return (
    <div className="p-8 max-w-6xl mx-auto space-y-8">
      <div className="space-y-2">
        <h2 className="text-3xl font-bold text-slate-900">SEO Audit</h2>
        <p className="text-slate-500">Paste your content or HTML snippet to identify optimization opportunities.</p>
      </div>
        ...
        <div className="space-y-4">
          <div className="bg-white p-1 rounded-2xl shadow-sm border border-slate-200">
            <textarea
              ...
              className="w-full h-96 p-6 rounded-xl outline-none resize-none text-slate-700 text-base leading-relaxed focus:ring-2 focus:ring-brand-500 focus:rounded-xl transition-all"
            />
          </div>
          <button
            onClick={handleAudit}
          ...
          {result ? (
            <div className="animate-fadeIn space-y-6">
              ...
              <div className="bg-white p-8 rounded-2xl shadow-sm border border-slate-100 flex items-center justify-between">
                <div>
                  <h3 className="text-lg font-semibold text-slate-700">SEO Score</h3>
                  <p className="text-slate-500 text-sm mt-1">Based on keyword usage, readability, and structure.</p>
                </div>
              ...
              <div className="space-y-3">
                <h4 className="font-semibold text-slate-900 ml-1">Detailed Insights</h4>
                {result.issues.map((issue, idx) => (
                  <div key={idx} className="bg-white p-5 rounded-xl shadow-sm border border-slate-100 border-l-4"
                    ...
                    <div className="flex items-start gap-3">
                      ...
                      <div>
                        ...
                        <p className="text-slate-500 text-sm mt-1">Tip: {issue.recommendation}</p>
                      </div>
                    </div>
                  </div>
                ))}
