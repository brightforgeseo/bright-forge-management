...
const ChatTodoList: React.FC<ChatTodoListProps> = ({ currentUser, addToast, onClose }) => {
  ...
  return (
    <div className="fixed inset-0 bg-black/40 flex items-center justify-center z-50 p-4" onClick={onClose}>
      <div
        className="bg-portal-surface rounded-2xl shadow-2xl w-full max-w-lg max-h-[80vh] flex flex-col overflow-hidden"
        onClick={e => e.stopPropagation()}
      >
        {/* Header */}
        <div className="flex items-center justify-between px-5 py-4 border-b border-white/[0.07] bg-gradient-to-r from-violet-500 to-purple-600">
          <div className="flex items-center gap-3">
            <div className="w-9 h-9 bg-portal-surface/20 rounded-xl flex items-center justify-center">
          ...
          <button onClick={onClose} className="p-1.5 hover:bg-portal-surface/20 rounded-lg transition-colors">
            <X className="w-5 h-5 text-white" />
          </button>
        </div>

        {/* Add new todo */}
        <div className="px-5 py-3 border-b border-white/[0.07] bg-portal-dark">
          ...
          <div className="flex items-center gap-2 mb-2">
            <button
              ...
              className={`flex items-center gap-1 px-2 py-1 text-xs rounded-lg transition-colors ${
                showNotesInput ? 'bg-violet-100 text-violet-700' : 'text-portal-soft hover:bg-portal-surface2'
              }`}
            >
          ...
          {showNotesInput && (
            <textarea
              ...
              rows={3}
              className="w-full px-3 py-2 text-sm border border-white/[0.07] rounded-lg focus:outline-none focus:ring-2 focus:ring-violet-300 focus:border-violet-400 resize-none mb-2"
            />
          )}
          {/* Client board selector */}
          <div className="flex items-center gap-2 mb-2">
            <span className="text-xs text-portal-soft whitespace-nowrap">Client:</span>
            <select
              value={selectedBoardId}
              onChange={e => setSelectedBoardId(e.target.value)}
              className="flex-1 px-2 py-1 text-xs border border-white/[0.07] rounded-lg bg-portal-surface focus:outline-none focus:ring-1 focus:ring-violet-300"
            >
          ...
          {/* Assign to member */}
          <div className="flex items-center gap-2 mb-2">
            <span className="text-xs text-portal-soft whitespace-nowrap">Assign to:</span>
            <select
          ...
          {selectedBoardId && availableGroups.length > 0 && (
            <div className="flex items-center gap-2">
              <span className="text-xs text-portal-soft whitespace-nowrap">Category:</span>
              <select
          ...
          <button
            ...
            className={`flex-1 px-4 py-2.5 text-sm font-medium transition-colors ${
              filter === 'active'
                ...
                : 'text-portal-soft hover:text-portal-text'
            }`}
          >
            Active ({activeCount})
          </button>
        ...
        <div className="flex-1 overflow-y-auto px-5 py-3 space-y-2">
          {loading ? (
            ...
            <div className="text-center py-12">
              <ListTodo className="w-10 h-10 text-portal-text mx-auto mb-3" />
              ...
              <div
                ...
                className={`group flex items-start gap-3 p-3 rounded-xl border transition-all ${
                  todo.is_completed
                    ...
                    : 'bg-portal-surface border-white/[0.07] hover:border-violet-300 hover:shadow-lg shadow-black/20'
                }`}
              >
                {/* Checkbox */}
                <button
                  ...
                  className={`flex-shrink-0 w-5 h-5 mt-0.5 rounded-full border-2 flex items-center justify-center transition-all ${
                    completingIds.has(todo.id)
                      ...
                      : 'border-white/[0.07] hover:border-violet-500'
                  }`}
                >
                  {completingIds.has(todo.id) ? (
                ...
                <div className="flex-1 min-w-0">
                  <p className={`text-sm ${todo.is_completed ? 'text-portal-soft line-through' : 'text-portal-text'}`}>
                    {todo.text}
                  </p>
                  {todo.notes && (
                    <div className="mt-1">
                      ...
                      {expandedNotes.has(todo.id) && (
                        <p className="mt-1 text-xs text-portal-soft bg-portal-dark rounded-lg p-2 whitespace-pre-wrap">
                          {todo.notes}
                        </p>
                      )}
                    </div>
                  ...
                  <div className="flex items-center gap-2 mt-1 flex-wrap">
                    <span className="text-[11px] text-portal-soft">
                ...
                <button
                  ...
                  className="flex-shrink-0 p-1 text-portal-text hover:text-red-500 opacity-0 group-hover:opacity-100 transition-all"
                  title="Delete"
                >
