...
export function startNotificationSession({auth,onSession,onInvalidate,bind,cleanup,intervalMs=60000}) {
  let generation=0, identity=null, stopped=false, paused=false;
  ...
  return {check, pause(){paused=true;generation++;identity=null;onInvalidate();}, resume(){paused=false;void check();}, stop(){stopped=true;generation++;subscription.unsubscribe();if(timer)clearInterval(timer);}};
}
